...
PTR(Val) Var::interp(PTR(Env) env) {
    PTR(Val) val = env->lookup(this->var);
    if (val == nullptr) {
...
PTR(Val) _Let::interp(PTR(Env) env) {
    PTR(Val) rhsValue = this->head->interp(env); // Evaluate the right-hand side expression in the current environment to get the value.
    PTR(Env) newEnv = NEW(ExtendedEnv)(this->varName, rhsValue, env); // Extend the environment with the new variable binding.
    return this->body->interp(newEnv); // Evaluate the body with the new, extended environment.
}
