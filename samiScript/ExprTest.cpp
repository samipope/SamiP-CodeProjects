...
TEST_CASE("interp tests", "All Expressions") {
    ...
    SECTION("Let interp"){
        CHECK(parse_str("_let x = 5 _in x")->interp(Env::empty)->equals(NEW(NumVal)(5)));
        CHECK(parse_str("_let x = 5 _in x + 3")->interp(Env::empty)->equals(NEW(NumVal)(8)));
        CHECK(parse_str("_let x = 5 _in _let y = x + 2 _in y + x")->interp(Env::empty)->equals(NEW(NumVal)(12)));
        CHECK(parse_str("_let x = 10 _in _let y = x + 1 _in _let z = y + 1 _in x + y + z")->interp(Env::empty)->equals(NEW(NumVal)(33)));
        CHECK(parse_str("_let x = 5 _in _let x = 3 _in x")->interp(Env::empty)->equals(NEW(NumVal)(3)));
...
TEST_CASE("Professor's given tests") {
    ...
    SECTION("Given Tests Assignment 3") {
        ...
        CHECK_THROWS_WITH(NEW(Var)("x")->interp(Env::empty), "free variable: x");
    }
